...
#' @param burn_dist The number of units (meters, feet) to use for burning-in the stream network.
...
hydroProcess <- function(DEM, breach_dist, streams = NULL, burn_dist = 10, save_path = NULL)
{
  ...
  suppressWarnings(unlink(temp_dir, recursive = TRUE, force = TRUE)) #in case the function ran in the previous session and things need to be cleared
  dem_path <- DEM
  ...
  if (!is.null(streams)){ #load streams, process to raster, and burn-in the DEM
    ...
    streams_input <- (streams_input/streams_input) * burn_dist #Make each cell value = 20 to later burn in a 20 unit depression
