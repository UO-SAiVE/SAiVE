...
test_that("Basins are created", {
  ...
  file.copy(test_path("fixtures/hydroProcess_createStreams_data", "basin_test_rast.tif"), paste0(tempdir(), "/drainageBasins_test/basin_test_rast.tif"))
  res <- drainageBasins(DEM = paste0(tempdir(), "/drainageBasins_test/basin_test_rast.tif"), streams = test_path("fixtures/hydroProcess_createStreams_data", "water_flow.shp"), breach_dist = 500, threshold = 200, points = test_path("fixtures/hydroProcess_createStreams_data", "basin_pts.shp"), points_name_col = "ID", save_path = paste0(tempdir(), "/drainageBasins_test/output"), overwrite=TRUE) #These are relatively high values for the size of the DEM, but necessary for reproducibility.
