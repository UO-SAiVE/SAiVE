...
test_that("correct stream network is created", {
  if (whitebox::check_whitebox_binary()){
    dem <- hydroProcess(system.file("extdata/basin_rast.tif", package = "SAiVE"), 500, system.file("extdata/streams.gpkg", package = "SAiVE"), 200) #These are relatively high values for the size of the DEM, but necessary for reproducibility.
